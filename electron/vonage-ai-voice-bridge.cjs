...
const TLS_KEY_PATH = path.join(PROJECT_ROOT, 'config', 'security', 'localhost.key.pem')
const TLS_CERT_PATH = path.join(PROJECT_ROOT, 'config', 'security', 'localhost.cert.pem')
...
function loadDotEnv() {
  ...
  for (const line of text.split('\n')) {
    const m = envLinePattern.exec(line)
...
function createBridge(getEnv) {
  function getKeyBase() {
    const env = getEnv()
    // SECURITY (CRIT-08): Removed VITE_ prefixed env vars (inlined in bundles).
    // Only read server-side env vars without VITE_ prefix.
    const key = (env.OPENAI_API_KEY || '').trim()
    const base = (env.OPENAI_BASE_URL || 'https://api.openai.com/v1').replace(/\/$/, '')
    return { key, base, env }
  }

  const server = https.createServer(getLocalTlsOptions(), (req, res) => {
  ...
  wss.on('connection', (ws) => {
    ...
    async function processChunk() {
      ...
      try {
        ...
        state.buf = state.buf.subarray(Math.max(take - 16000, 0))
    ...
    ws.on('message', (data, isBinary) => {
      if (isBinary && Buffer.isBuffer(data)) {
        ...
        if (state.buf.length > 960000) {
          state.buf = state.buf.subarray(- 480000)
        }
        return
      }
      try {
        ...
        if (typeof data === 'string') {
          ...
          textPayload = data.toString('utf8')
        } else if (data instanceof ArrayBuffer) {
          textPayload = Buffer.from(data).toString('utf8')
        } else if (ArrayBuffer.isView(data)) {
          textPayload = Buffer.from(data.buffer, data.byteOffset, data.byteLength).toString('utf8')
        } else {
          return
        }
...
function startStandalone() {
  ...
  server.listen(port, '0.0.0.0', () => {
    console.log(`[vonage-ai-voice-bridge] listening on https://0.0.0.0:${String(port)}${WS_PATH} (WebSocket)`)
